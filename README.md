...
* **核心语言**：Python 3.10+
* **GUI 框架**：PyQt6 (实现 WCAG 2.1 AA 标准对比度)
* **网络通信**：TCP Socket P2P 协议 (Host-Guest 模式)
* **AI 服务**：OpenAI API 兼容协议 (默认模型 `gpt-4o-mini`)
...
### 2. 配置 AI API (仅 Host 端)

启动应用后，通过菜单栏 `文件 -> API 配置` (或 `Ctrl+K`) 配置您的 API Key。配置将安全地存储在本地 `config.json` 中。
...
* **零中转服务器**：聊天记录仅通过 P2P 协议在双方设备间传输，不经过第三方服务器。
